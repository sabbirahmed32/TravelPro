...
@section('header', 'Edit Travel Package')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-gray-900">Edit Travel Package</h1>
                    <p class="mt-1 text-sm text-gray-500">Update package information</p>
                </div>
                <div>
                    <a href="{{ route('admin.travel-packages.show', $travelPackage) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
        ...
        <div class="bg-white shadow sm:rounded-lg">
            <form method="POST" action="{{ route('admin.travel-packages.update', $travelPackage) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="px-4 py-5 sm:p-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" id="name" name="name" required
                                   ...
                                   value="{{ old('name', $travelPackage->title) }}" placeholder="e.g., Dubai Adventure Package">
                            @error('name')
                        ...
                        <div>
                            ...
                            <select id="destination" name="destination" required
                                ...
                                <option value="dubai" {{ old('destination', $travelPackage->destination) == 'dubai' ? 'selected' : '' }}>Dubai</option>
                                <option value="singapore" {{ old('destination', $travelPackage->destination) == 'singapore' ? 'selected' : '' }}>Singapore</option>
                                <option value="malaysia" {{ old('destination', $travelPackage->destination) == 'malaysia' ? 'selected' : '' }}>Malaysia</option>
                                <option value="thailand" {{ old('destination', $travelPackage->destination) == 'thailand' ? 'selected' : '' }}>Thailand</option>
                                <option value="europe" {{ old('destination', $travelPackage->destination) == 'europe' ? 'selected' : '' }}>Europe</option>
                                <option value="usa" {{ old('destination', $travelPackage->destination) == 'usa' ? 'selected' : '' }}>USA</option>
                            </select>
                            @error('destination')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" id="duration" name="duration" required min="1"
                                   ...
                                   value="{{ old('duration', $travelPackage->duration_days) }}" placeholder="e.g., 7">
                            @error('duration')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="status" name="status" required
                                ...
                                <option value="active" {{ old('status', $travelPackage->is_active ? 'active' : 'inactive') == 'active' ? 'selected' : '' }}>Active</option>
                                <option value="inactive" {{ old('status', $travelPackage->is_active ? 'active' : 'inactive') == 'inactive' ? 'selected' : '' }}>Inactive</option>
                            </select>
                            @error('status')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="mt-6">
                        ...
                        <textarea id="description" name="description" rows="4" required
                                  ...
                                  placeholder="Describe the travel package, highlights, inclusions, etc.">{{ old('description', $travelPackage->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" id="price" name="price" required min="0" step="0.01"
                                   ...
                                   value="{{ old('price', $travelPackage->price) }}" placeholder="e.g., 1299.99">
                            @error('price')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select id="currency" name="currency" required
                                ...
                                <option value="BDT" {{ old('currency', 'USD') == 'BDT' ? 'selected' : '' }}>BDT</option>
                            </select>
                            @error('currency')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="date" id="start_date" name="start_date"
                                   ...
                                   value="{{ old('start_date', $travelPackage->start_date) }}">
                            @error('start_date')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" id="end_date" name="end_date"
                                   ...
                                   value="{{ old('end_date', $travelPackage->end_date) }}">
                            @error('end_date')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                    <a href="{{ route('admin.travel-packages.show', $travelPackage) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 shadow-sm text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 mr-3">
                        Cancel
                    </a>
